// Code generated by go-swagger; DO NOT EDIT.

package developers

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"
)

// NewV2AvailablePropertiesParams creates a new V2AvailablePropertiesParams object
// with the default values initialized.
func NewV2AvailablePropertiesParams() *V2AvailablePropertiesParams {

	return &V2AvailablePropertiesParams{

		timeout: cr.DefaultTimeout,
	}
}

// NewV2AvailablePropertiesParamsWithTimeout creates a new V2AvailablePropertiesParams object
// with the default values initialized, and the ability to set a timeout on a request
func NewV2AvailablePropertiesParamsWithTimeout(timeout time.Duration) *V2AvailablePropertiesParams {

	return &V2AvailablePropertiesParams{

		timeout: timeout,
	}
}

// NewV2AvailablePropertiesParamsWithContext creates a new V2AvailablePropertiesParams object
// with the default values initialized, and the ability to set a context for a request
func NewV2AvailablePropertiesParamsWithContext(ctx context.Context) *V2AvailablePropertiesParams {

	return &V2AvailablePropertiesParams{

		Context: ctx,
	}
}

// NewV2AvailablePropertiesParamsWithHTTPClient creates a new V2AvailablePropertiesParams object
// with the default values initialized, and the ability to set a custom HTTPClient for a request
func NewV2AvailablePropertiesParamsWithHTTPClient(client *http.Client) *V2AvailablePropertiesParams {

	return &V2AvailablePropertiesParams{
		HTTPClient: client,
	}
}

/*V2AvailablePropertiesParams contains all the parameters to send to the API endpoint
for the v2 available properties operation typically these are written to a http.Request
*/
type V2AvailablePropertiesParams struct {
	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithTimeout adds the timeout to the v2 available properties params
func (o *V2AvailablePropertiesParams) WithTimeout(timeout time.Duration) *V2AvailablePropertiesParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the v2 available properties params
func (o *V2AvailablePropertiesParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the v2 available properties params
func (o *V2AvailablePropertiesParams) WithContext(ctx context.Context) *V2AvailablePropertiesParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the v2 available properties params
func (o *V2AvailablePropertiesParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the v2 available properties params
func (o *V2AvailablePropertiesParams) WithHTTPClient(client *http.Client) *V2AvailablePropertiesParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the v2 available properties params
func (o *V2AvailablePropertiesParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WriteToRequest writes these params to a swagger request
func (o *V2AvailablePropertiesParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
